...
void root(double &destination, int const base, double const epsilon) {
    destination = static_cast<double>(base)/2;
    double epsilon_x = (static_cast<double>(base)-destination*destination)/(2*destination);
    while (epsilon_x > (epsilon/2)*(epsilon/2) || epsilon_x < -(epsilon/2)*(epsilon/2))
    {
        destination = destination+epsilon_x;
        epsilon_x = (static_cast<double>(base)-destination*destination)/(2*destination);
