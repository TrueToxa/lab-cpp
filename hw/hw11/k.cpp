/* "Теперь от вас требуют использовать оператор switch-case и enum Command" 
- У меня сразу возникла идея так сделать :| */
...
enum Command { FINISH, INSERT, MIN, MAX, FIND, NEXT, DELETE, DESTROY, START };.
...
void print(Node *&tree, TraversalType  type)
{
    if (tree == nullptr)
    {
        std::cout << std::endl;
...
int main()
{
    ...
    while (std::cin >> command)
    {
        ...
        case PRINT:
        {
            int type;
            std::cin >> type;
            print(tree, static_cast<TraversalType >(type));
            break;
        }
        case DELETE:
        {
